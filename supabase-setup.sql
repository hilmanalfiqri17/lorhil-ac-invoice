-- ============================================================
-- LORHIL AC ONLINE V5 - SUPABASE DATABASE SETUP
...
create policy "items_insert_own" on public.invoice_items
...
create policy "items_update_own" on public.invoice_items
...
create policy "items_delete_own" on public.invoice_items
for delete to authenticated using (user_id = auth.uid());
...
-- Sequence tidak diakses langsung dari browser.
...
begin
  ...
  v_status := case when v_total > 0 and v_paid >= v_total then 'Lunas'
                   when v_paid > 0 then 'DP'
                   else 'Belum Lunas' end;

  if p_invoice_id is null then
    v_number := public.next_invoice_number(extract(year from p_work_date)::integer);
    insert into public.invoices(
      user_id, invoice_number, work_date, work_time, customer_name,
      customer_phone, customer_address, subtotal, discount, total,
      paid, balance, status, notes
    ) values(
      v_user, v_number, p_work_date, p_work_time, btrim(p_customer_name),
      nullif(btrim(coalesce(p_customer_phone,'')),''), nullif(btrim(coalesce(p_customer_address,'')),''),
  ...
  select
    v_id, v_user, btrim(x->>'description'),
...
begin
  ...
  update public.invoices set
    ...
    status=case when v_total>0 and v_paid>=v_total then 'Lunas'
                when v_paid>0 then 'DP'
                else 'Belum Lunas' end
  where id=p_invoice_id and user_id=v_user
...
-- Aktifkan Realtime untuk tabel invoice bila belum aktif.
do $$
begin
  if not exists (
